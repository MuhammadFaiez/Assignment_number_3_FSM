...
public class Enemy_FSM : MonoBehaviour
{
    ...
    public enum ENEMY_STATE { ethan, Movestart, HandsonEthan, LegsFoldEthan, Levelup };
    ...
    public ENEMY_STATE CurrentState
    {
        ...
        set
        {
            ...
            switch (currentState)
            {
                case ENEMY_STATE.ethan:
                    StartCoroutine(ethan());
                    break;
                case ENEMY_STATE.Movestart:
                    StartCoroutine(Movestart());
                    break;

                case ENEMY_STATE.HandsonEthan:
                    StartCoroutine(Handson());
                    break;

                case ENEMY_STATE.Levelup:
    ...
    void Start()
    {
        ...
        CurrentState = ENEMY_STATE.ethan;
    }

    public IEnumerator ethan()
    {
        while (currentState == ENEMY_STATE.ethan)
        {
    ...
    public IEnumerator Movestart()
    {
        ...
        while (currentState == ENEMY_STATE.Movestart)
        {
            ...
            if (agent.remainingDistance <= agent.stoppingDistance)
            {
                ...
                else
                {
                    ...
                    CurrentState = ENEMY_STATE.Levelup;
                }
                yield break;
            }
    ...
    public IEnumerator Handson()
    {
        Debug.Log("Enemy Chase starting");
        //Again we shall start with a loop

        while (currentState == ENEMY_STATE.HandsonEthan)
        {
            //In this case, let us keep sensitivity LOW
            checkMyVision.sensitivity = CheckMyVision.enmSensitivity.LOW;
